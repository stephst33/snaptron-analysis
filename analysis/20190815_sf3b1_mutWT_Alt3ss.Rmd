...
#first, convert chromStart into numerics, sort Alt3ss data frames by chrom and chromStart, then save as files

Alt3ss1$chromStart <- as.numeric(Alt3ss1$chromStart)
Alt3ss2$chromStart <- as.numeric(Alt3ss2$chromStart)

Alt3ss1 %>% arrange(chrom, chromStart) %>% write.table("Alt3ss1.bed", append= FALSE, quote=FALSE, na= ".", row.names=FALSE,col.names=FALSE,sep="\t")

Alt3ss2 %>% arrange(chrom, chromStart) %>% write.table("Alt3ss2.bed", append= FALSE, quote=FALSE, na= ".", row.names=FALSE,col.names=FALSE,sep="\t")


#then use bedtools:closest

system("bedtools closest -a Alt3ss1.bed -b 3SS_output.bed -s -D ref > closest_output1.bed")
system("bedtools closest -a Alt3ss2.bed -b 3SS_output.bed -s -D ref > closest_output2.bed")
...
ggplot(ToPlot, aes(distance, proportion, color=allele, fill= allele)) +
  geom_histogram(alpha=0.5, position= "dodge", stat="identity") +
